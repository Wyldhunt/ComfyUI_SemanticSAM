...
class SemanticSAMLoader:
    ...
    def main(self, model_name, segmenter="OneFormer ADE20K (Swin-T)"):
        ...
        predictor = SemanticSAMPredictor(
            build_semantic_sam(model_type=model_name, ckpt=ckpt_path)
        )
...
class SemanticSAMSegment:
    ...
    def prepare_image(self, image: torch.Tensor):
        ...
        tensor_image = image[0].permute(2, 0, 1)
...
def masks2rgba(image: torch.Tensor, masks: List[np.ndarray]):
    ...
    for mask in masks:
        ...
        if mask_arr.shape != (image.shape[0], image.shape[1]):
            mask_arr = cv2.resize(
                mask_arr, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST
            )
        mask_arr = (mask_arr > 0.5).astype(np.float32)
        rgba_tensor = mask2rgba(image, mask_arr)
    ...
    if masks_tensor:
        ...
        masks_tensor = torch.zeros((0, image.shape[0], image.shape[1]), dtype=torch.float32)
        rgba_imgs = torch.zeros((0, image.shape[0], image.shape[1], 4), dtype=torch.float32)

...
class UniversalSegmenter:
    ...
    The segmenter loads a "universal" segmentation model such as OneFormer from
    the Transformers library and exposes a helper for obtaining semantic class
    ids for an input image. Only PyTorch kernels are required – no custom CUDA
    ...
    def __init__(self, config: Optional[UniversalSegmenterConfig] = None):
        ...
        # Cache whether the processor expects a task prompt (OneFormer families)
        # or not.
